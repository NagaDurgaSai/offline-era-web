...
export default function LanAnimation() {
  ...
  useEffect(() => {
    ...
    // iOS Safari needs explicit context options
    ...
    // Defer resize to next frame so getBoundingClientRect is accurate
    requestAnimationFrame(() => {
      resize()
      window.addEventListener('resize', resize)
    })

    const getPos = (node: typeof NODES[0], w: number, h: number) => ({
    ...
    const draw = () => {
      ...
      if (frameRef.current % 70 === 0) spawnPacket()
      if (frameRef.current % 120 === 0) spawnPacket()

      EDGES.forEach(([ai, bi], idx) => {
        const pa = getPos(NODES[ai], w, h), pb = getPos(NODES[bi], w, h)
      ...
      packetsRef.current.forEach(p => {
        ...
        grd.addColorStop(0, `rgba(184,255,87,${alpha * 0.4})`); grd.addColorStop(1, 'rgba(184,255,87,0)')
        ctx.beginPath(); ctx.arc(x, y, 12, 0, Math.PI * 2); ctx.fillStyle = grd; ctx.fill()
        if (p.progress > 0.92) {
      ...
      NODES.forEach((node, idx) => {
        ...
        ctx.strokeStyle = `rgba(184,255,87,${0.06 + breathe * 0.06 + ns.glow * 0.3})`
        ...
        ctx.fillStyle = `rgba(255,255,255,${0.55 + ns.glow * 0.45})`
        ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(node.icon, x, y)
    ...
    spawnPacket(); spawnPacket(); spawnPacket()
    draw()
